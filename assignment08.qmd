...
graph_pca_cluster <- function(df, clust){
  ...
  kmeans_spec <- kmeans() %>%
    set_engine("stats", nstart = 100) %>%
    set_mode("unsupervised") %>%
    set_args(centers = clust)
  ...
  # Fitting data
  kmeans <- kmeans_wflow %>%
    fit(data = baked_data) # Using transformed data
  
  # Graphing PCA1 and PCA2 with color coded by cluster assigned.
  graph <- bind_cols(
    select(baked_data, PC1, PC2),
    cluster = kmeans %>%
...
```{r}
# Pre processing the training data using the recipe
bills_processed <- prep(bills_recipe) %>%
  bake(new_data = NULL)

# Re-creating the recipe with domain-specific stop words
bills_recipe_2 <- recipe(passed ~ description, data = bills_train) %>%
  step_tokenize(description) %>%
  step_stopwords(description, custom_stopword_source = c("1","2","3","4","act", "addit", "bill", "certain", "congress")) %>%
  step_stem(description) %>%
  step_tokenfilter(description, max_tokens = 200) %>%
  step_tfidf(description) 
```
